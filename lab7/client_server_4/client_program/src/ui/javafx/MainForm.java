...
public class MainForm {
    ...
    private HBox createBottomToolbar() {
        ...
        btnInfo.setOnAction(event -> UiHelper.showAlert(Alert.AlertType.INFORMATION, UI.getI18n("form_info").get(),
                UI.getI18n("info_text_item1").get() + ": " + personsTable.getPersons().size() + ", " +
                UI.getI18n("info_text_item2").get() + ": " + personsTable.getPersons().stream().filter(p -> p.getUserId().equals(Identification.getIdentification().getUserInfo().getId())).count()
        ));
