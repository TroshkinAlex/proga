...
public class DbConnector {
    ...
    /*
    static String serverName = "localhost:5434";
    static String dbName = "studs";
    static String dbUser = "s333304";
    static String dbPass = "ppZc9BQeCu3ZTcYo";
    */
    ...
    private static final int CONDITION_IF_MAX = 1;
    private static final int CONDITION_IF_MIN = 2;
    private static final int CONDITION_NO = 0;

    public int addPersonIfMax(Person person) throws Exception {
        return addPerson(person, CONDITION_IF_MAX);
    }

    public int addPersonIfMin(Person person) throws Exception {
        return addPerson(person, CONDITION_IF_MIN);
    }

    public int addPerson(Person person) throws Exception {
        return addPerson(person, CONDITION_NO);
    }

    public int addPerson(Person person, int condition) throws Exception {
        Connection connection = null;
        PreparedStatement preparedStatement = null;
        ResultSet resultSet = null;
        try {
...
//            Добавление точно ненулевых элементов
            ...
            preparedStatement = connection.prepareStatement(
                    "insert into persons(name, coordinates_x, coordinates_y, height, creationDate, eyeColor, hairColor, nationality, location_id, user_id) " +
                         "select ?, ?, ?, ?, ?, ?, ?, ?, ?, ? " +
                         (condition != CONDITION_NO ? "where not exists (select 1 from persons where height " + (condition == CONDITION_IF_MAX ? " >= " : " <= ") + " ?)" : "")
    ...
    public void removeGreaterHeight(Long height, Integer userId) throws Exception {
        Connection connection = null;
        PreparedStatement preparedStatement = null;
        try {
            connection = getConnection();
            preparedStatement = connection.prepareStatement("delete from persons where height > ? and user_id = ?");
            preparedStatement.setLong(1, height);
            preparedStatement.setInt(2, userId);
            preparedStatement.execute();
            preparedStatement.close();

            preparedStatement = connection.prepareStatement("delete from person_location where id not in (select location_id from persons)");
            preparedStatement.executeUpdate();
    ...
    public void removeAll(Integer userId) throws Exception {
        ...
        try {
            connection = getConnection();
            preparedStatement = connection.prepareStatement("delete from persons where user_id = ?");
    ...
    public void initializeCollection() throws Exception {
        ...
        try {
            ...
            resultSet = preparedStatement.executeQuery();
            synchronized (PersonStorage.getPersons()) {
                PersonStorage.getPersons().clear();
